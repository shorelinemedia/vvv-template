...
if [[ ! -d "/home/vagrant/liquidprompt" ]]; then
  noroot mkdir ~/liquidprompt
  noroot git clone https://github.com/nojhan/liquidprompt.git ~/liquidprompt
  noroot source ~/liquidprompt/liquidprompt
  # Copy liquidprompt config
  noroot cp ~/liquidprompt/liquidpromptrc-dist ~/.config/liquidpromptrc

  # Add to .bashrc
  noroot cat <<EOF >> ~/.bashrc
...
EOF
  ...
  PATHLENGTH=16
  noroot sed "s/LP_PATH_LENGTH\=[0-9]*/LP_PATH_LENGTH=${PATHLENGTH}/" ~/.config/liquidpromptrc
fi
